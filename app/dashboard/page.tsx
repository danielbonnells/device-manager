...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function getMe() {
      const res = await fetch('http://localhost:5231/api/Users/me', {
        method: 'GET',
  ...
  const handleRegistrationResponse = async (response: any) => {
   ...
   async function registerDevice(e: any) {
        ...
        const res = await fetch('http://localhost:5231/api/Devices/register', {
        method: 'POST',
        credentials: 'include',
        body: formData
  ...
  return (
    <div className='flex flex-wrap flex-col p-8'>
      ...
      <form autoComplete="off" className='flex flex-col m-4 p-8 rounded-2xl bg-gray-50 drop-shadow-2xl' onSubmit={registerDevice}>
        ...
        <label htmlFor="UniqueId" className='font-bold mt-4'>Unique ID Code</label>
        <input id='UniqueId' name='UniqueId' type='text' className='border-2 p-2 rounded-md bg-white focus:ring-green-500 focus:outline-none focus:ring-2 focus:ring-offset-0'></input>
